...
async def get_random_phrase_async(trigger: str, mood: str = None) -> str | None:
    if _get_random_phrase:
        return await _get_random_phrase(trigger, mood)
    return None


async def get_rank_phrase_async(rank_name: str) -> str | None:
    if _get_rank_phrase:
        return await _get_rank_phrase(rank_name)
    return None


async def get_streak_achievement_async(day: int) -> str | None:
...
async def process_message(message: Message):
    ...
    for min_streak, rank_name in sorted(rank_mapping.items()):
        if current_streak >= min_streak and user['rank'] != rank_name:
            ...
            rank_phrase = await get_rank_phrase_async(rank_name)
            if rank_phrase:
    ...
    if current_streak in achievement_days:
        ...
        if not already:
            phrase = await get_streak_achievement_async(current_streak)
            if not phrase:
...
async def check_inactive_users(bot: Bot):
    ...
    for user in inactive:
        ...
        # 24-48 часов: отправляем фразу
        if 24 <= hours_since < 48:
            phrase = await get_random_phrase_async("1_DAY_INACTIVE")
            if phrase:
        ...
        # 48+ часов: сбрасываем стрик, даём шанс на искупление
            ...
            if shield_active:
                ...
                # Случайно отправляем картинку или мем
                if random.choice([True, False]):
                    # Пытаемся взять картинку или мем
                    picture = get_random_picture("2_PLUS_DAYS_INACTIVE", "SARCASTIC", is_meme=True)
                    if not picture:
                        picture = get_random_picture("2_PLUS_DAYS_INACTIVE", "SARCASTIC", is_meme=False)
                    
                    if picture:
                        await bot.send_photo(user['user_id'], photo=FSInputFile(picture))
                    else:
                        # Если картинки нет — отправляем фразу
                        phrase = await get_random_phrase_async("2_PLUS_DAYS_INACTIVE")
                        if phrase:
                            await bot.send_message(user['user_id'], phrase)
